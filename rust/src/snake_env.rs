...
impl SnakeEnv {
    ...
    fn place_food(&mut self) -> (i32, i32) {
        ...
        while self.food_idx < self.food_sequence.len() {
            ...
            self.food_idx += 1;
            if !snake_set.contains(&pos) { return pos; }
        }
        loop {
    ...
    pub fn step(&mut self, action: usize) -> (Vec<f32>, bool) {
        if action != OPPOSITE[self.direction] { self.direction = action; }
        ...
        if body.contains(&(nr, nc)) { return (self.get_obs(), true); }
        ...
        if (nr, nc) == self.food {
            ...
            self.snake.pop();
            self.hunger += 1;
            if self.hunger >= self.hunger_limit { return (self.get_obs(), true); }
        }
        self.steps += 1;
        if self.steps >= self.max_steps { return (self.get_obs(), true); }
        (self.get_obs(), false)
    }
    ...
    pub fn get_obs(&self) -> Vec<f32> {
        ...
        obs[16] = (hr - fr).max(0) as f32 / g;
        obs[17] = (fr - hr).max(0) as f32 / g;
        obs[18] = (hc - fc).max(0) as f32 / g;
        obs[19] = (fc - hc).max(0) as f32 / g;
        obs
    }
}
