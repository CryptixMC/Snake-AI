...
pub const GAME_W:  f32 = CELL * GRID as f32;
pub const GAME_H:  f32 = GAME_W;
pub const HUD_H:   f32 = 50.0;
pub const PANEL_X: f32 = GAME_W + 10.0;
pub const WIN_W:   f32 = PANEL_X + 370.0;
pub const WIN_H:   f32 = GAME_H + HUD_H;
...
fn rank_color(rank: usize, n: usize) -> Color {
    ...
    hsv(0.38, (1.0 - t * 0.85).max(0.12), (1.0 - t * 0.68).max(0.28))
}
...
impl Visualizer {
    ...
    pub async fn play_top_n(&self, params_list: &[Vec<f32>], generation: usize, fps: f64) -> i32 {
        ...
        loop {
            if is_key_pressed(KeyCode::Q) || is_key_pressed(KeyCode::Escape) { return -1; }
            ...
            if let Some(ref acts) = best_acts { self.draw_panel(acts); }
            ...
            if alive.iter().all(|&a| !a) {
                ...
                return envs.iter().map(|e| e.score).max().unwrap_or(0) as i32;
            }
        }
    }
    ...
    fn draw_game(&self, envs: &[SnakeEnv], alive: &[bool], colors: &[Color]) {
        ...
        for i in 0..=(self.grid_size as usize) {
            draw_line(i as f32 * c, 0.0, i as f32 * c, GAME_H, 1.0, grid_col);
            draw_line(0.0, i as f32 * c, GAME_W, i as f32 * c, 1.0, grid_col);
        }
        for i in (0..envs.len()).rev() {
            if !alive[i] { continue; }
            let (fr, fc) = envs[i].food;
            let col = colors[i];
            draw_circle(fc as f32 * c + c/2.0, fr as f32 * c + c/2.0, 4.0,
                Color::new(col.r*0.5, col.g*0.5, col.b*0.5, 1.0));
        }
        for i in (0..envs.len()).rev() {
            if !alive[i] { continue; }
            let col = colors[i];
            let head = Color::new((col.r*1.45).min(1.0),(col.g*1.45).min(1.0),(col.b*1.45).min(1.0),1.0);
            for (idx, &(r, cp)) in envs[i].snake.iter().enumerate() {
                draw_rectangle(cp as f32*c+1.0, r as f32*c+1.0, c-2.0, c-2.0, if idx==0 {head} else {col});
            }
        }
    }
    ...
    fn draw_panel(&self, acts: &crate::network::ForwardResult) {
        ...
        draw_text("NEURAL NETWORK", PANEL_X+100.0, 14.0, 14.0, dim);
    ...
    fn draw_connections(&self, acts: &crate::network::ForwardResult) {
        ...
        for i in 0..IN_SIZE {
            for j in 0..H_SIZE {
                ...
                draw_line(INP_X, self.inp_ys[i], HID_X, self.hid_ys[j], 1.0, Color::new(r,g,b,alpha));
            }
        }
        for j in 0..H_SIZE {
            for k in 0..OUT_SIZE {
                ...
                draw_line(HID_X, self.hid_ys[j], OUT_X, self.out_ys[k], 1.0, Color::new(r,g,b,alpha));
            }
        }
    }
    ...
    fn draw_neurons(&self, acts: &crate::network::ForwardResult) {
        ...
        let dim = Color::from_rgba(65,65,75,255);
        let food_lbls = ["^","v","<",">"];
        for i in 0..IN_SIZE {
            let clr = if i < 8 { lerp_col(WALL_DARK, WALL_BRIGHT, inp[i]) }
                      else if i < 16 { lerp_col(BODY_DARK, BODY_BRIGHT, inp[i]) }
                      else { lerp_col(FOOD_DARK, FOOD_BRIGHT, inp[i]) };
            draw_circle(INP_X, self.inp_ys[i], R_INP, clr);
            let lbl = if i < 16 { SENSOR_DIRS[i%8] } else { food_lbls[i-16] };
            draw_text(lbl, INP_X-14.0, self.inp_ys[i]+4.0, 11.0, dim);
        }
        ...
        for j in 0..H_SIZE {
            draw_circle(HID_X, self.hid_ys[j], R_HID, lerp_col(HID_DARK, HID_BRIGHT, hid[j]/hid_max));
        }
        for k in 0..OUT_SIZE {
            let clr = if k == action { Color::from_rgba(30,230,90,255) }
                      else { let v = (softmax[k]*160.0) as u8; Color::from_rgba(v,v,v,255) };
    ...
    fn draw_hud(&self, envs: &[SnakeEnv], alive: &[bool], colors: &[Color], generation: usize) {
        ...
        let n          = envs.len();
        draw_text(
            &format!("gen {}   best score {}   alive {}/{}", generation, best_score, n_alive, n),
            8.0, GAME_H+22.0, 16.0, Color::from_rgba(200,200,200,255),
        );
        let mut lx = 8.0f32;
        for i in 0..n.min(40) {
